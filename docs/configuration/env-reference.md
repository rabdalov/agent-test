...
**Правила доступа:**
1. **Администратор** (`ADMIN_ID`) — всегда имеет полный доступ
2. **Разрешённые** (`TLG_ALLOWED_ID`, `allowed_users`) — имеют доступ
3. **Отклонённые** (`denied_users`) — доступ запрещён (без уведомления админа)
4. **Новые пользователи** — доступ запрещён по умолчанию, администратор получает уведомление с кнопками "Добавить" / "Отклонить"

**Хранение пользователей:**
- `allowed_users` и `denied_users` хранятся в файле `users.json`
- Файл создаётся автоматически при первом добавлении пользователя
...
**Алгоритм проверки:**
```
if user_id == ADMIN_ID → доступ разрешён
if user_id in denied_users → доступ запрещён
if user_id in allowed_users → доступ разрешён
if user_id in TLG_ALLOWED_ID → доступ разрешён
→ доступ запрещён, уведомить администратора
```
...
При первом запуске:
1. Бот доступен только администратору (`ADMIN_ID`)
2. При попытке доступа нового пользователя — администратор получает уведомление
3. Администратор может добавить или отклонить пользователя через inline-кнопки
4. Добавленные пользователи сохраняются в `users.json`
5. При перезапуске бота списки загружаются из `users.json`
